...
| DELETE | `/api/progress` | 전체 진행률 초기화 | 🔒 |
...
| 메서드 | 엔드포인트 | 설명 | 인증 |
|--------|-----------|------|------|
| POST | `/api/feedback` | 피드백 제출 (1분당 3회) | 🔒 |
...
**테이블 목록**: `users`, `stories`, `story_pages`, `story_page_sentences`, `subscriptions`, `reading_progress`, `feedbacks`
...
| `test/feedback.e2e-spec.ts` | 7 | E2E |
| `src/subscription/subscription.service.spec.ts` | 8 | Unit |
| `src/feedback/feedback.service.spec.ts` | 4 | Unit |
...
| `20260304000000_add_feedbacks.sql` | `feedbacks` 테이블 신설 (고객 피드백 수집) |
